...
Bld_Ang(void)
{
  ...
  if (column_angle < 0)
    column_angle += NUMBER_OF_DEGREES;
  if (column_angle > NUMBER_OF_DEGREES - 1)
    column_angle -= NUMBER_OF_DEGREES;
  ...
  for (a = 101; a <= 200; a++)
  {
    lside[a] = fixdiv(row_table[a], _cos(l_angle));
    rside[a] = fixdiv(row_table[a], _cos(r_angle));
  }
...
void
draw_maze(int xview, int yview, int viewing_angle)
{
  ...
  case 0:
    ...
    for (last_ctr = 0, row = 101; row <= prm_window_bottom; row++, last_ctr++)
    {
      if (last_va != viewing_angle)
      {
        ...
        left_y = fixmul(fix_distance, _cos(degrees));
        ...
        right_y = fixmul(fix_distance, _cos(degrees));
...
int
getdistance(int degrees, int column_angle, int x, int y)
{
  int b, distance;

  b = degrees >> 9; // / 512;
  if (((b & 3) == 0) || ((b & 3) == 3))
    distance = (fixdiv((y - g_yview) >> SHIFT, _cos(degrees - (b << 11))) * _cos(column_angle) >> SHIFT);
  else
    distance = (fixdiv((x - g_xview) >> SHIFT, SIN(degrees - (b << 11))) * _cos(column_angle) >> SHIFT);
...
int
getd2(int x, int y, int xview, int yview)
{
  int a, b, c;
