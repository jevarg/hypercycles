...
level_def_show()
{
  ...
  printf("            # of Keys > %i\n", level_def.rings_req);
  gotoxy(1, 40);
  printf("Max Simul Riders > %i\n", level_def.max_sim_riders);
  gotoxy(2, 40);
  printf("Max Total Riders > %i\n", level_def.max_tot_riders);
  gotoxy(3, 40);
  printf("        Option 1 > %i\n", level_def.opt1);
  gotoxy(4, 40);
  printf("        Option 2 > %i\n", level_def.opt2);
...
object_def_edit()
{
  ...
  while (1)
  {
    ...
    {
      gotoxy(20, 20);
      cprintf("Save Entry(s) - Confirmation ([Y]/N) ?");
      a = getche();
      if (a == 'Y' || a == 'y' || a == 13)
        save_object_def();
...
level_def_edit()
{
  ...
  while (1)
  {
    ...
    {
      gotoxy(23, 20);
      cprintf("Save Entry - Confirmation ([Y]/N) ?");
      a = getche();
      if (a == 'Y' || a == 'y' || a == 13)
        save_level_def();
...
mainmenu(void)
{
  ...
  while (1)
  {
    clrscrn();
    gotoxy(0, 11);
    puts("HyperCycles Level Builder - (c) 1995 Aclypse Corporation ");
    gotoxy(1, 11);
    puts("             For Aclypse Internal Use Only");

    gotoxy(3, 11);
    puts("      ***************** Main Menu *****************");
    gotoxy(5, 32);
    cprintf("<<< Level # %i of %i %i>>>", level_num, total_level_def, sizeof(single));
    gotoxy(7, 20);
    puts("  1 - Level Definitions");
    gotoxy(9, 20);
    puts("  2 - Object Definitions for Levels");
    gotoxy(11, 20);
    puts("  S - Select Level");
    gotoxy(12, 20);
    puts("  A - Add Level");
    gotoxy(13, 20);
    puts("  C - Clear Object Definitions");
    gotoxy(15, 20);
    puts("  X - Exit");
    ...
    while (!mmloop)
    {
      ...
      case 'c':
        gotoxy(19, 20);
        cprintf("Clear Confirmation (Y/N) ?");
        a = getche();
        if (a == 'Y' || a == 'y')
        {
          clear_level_def();
